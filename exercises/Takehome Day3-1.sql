...
/* Solution */
select Branch,sum(revenue-Cost) from bank_branch_pl group by Branch having sum(revenue-Cost)>0;
...
/* Solution */
select Product,Month,sum(Quantity) as total_quantity from bank_inventory_pricing group by Product,Month order by Product,Month;

# Question 4:
# 4)  Find the maximum profit(Estimated_sale_price-Purchase_cost) for each product (Hint: Do not display loss).
/* Solution */
select Product,max(ifnull(Estimated_sale_price,0)-ifnull(Purchase_cost,0)) maximum_profit from bank_inventory_pricing group by Product having maximum_profit>0;
...
select Product,Month,sum(Quantity) from bank_inventory_pricing group by Product,Month having sum(Quantity)>6;
...
select Branch from bank_branch_pl where (revenue-Cost)>Estimated_profit group by Branch;
...
select Branch,sum(Estimated_profit) "total estimated price",sum(revenue-Cost) "real profit" from bank_branch_pl group by Branch having sum(revenue-Cost)>0;
...
select Product,Banker,Branch,count(*) from bank_branch_pl where (revenue-Cost)<0 group by Product,Banker,Branch;

# Question 10:
# 10)  Display the name of the branch and product that has earned the least profit.
select Branch,Product from bank_branch_pl where revenue-Cost>0 order by (revenue-Cost) limit 1;
...
select Branch,Product,sum(revenue-Cost) from bank_branch_pl 
where revenue>Cost group by Branch,Product 
...
select Product,Branch,sum(Estimated_profit),sum(revenue-Cost) from bank_branch_pl group by Product,Branch 
...
select Branch,Product from bank_branch_pl group by Branch,Product 
having sum(revenue-(Cost-(Cost*0.3)))>sum(Estimated_profit);
